...
int main() {
    ...
    }
    {
        MyTime timer0;
        timer0.start();
        auto ret = cudaMalloc((void**)&dfp32_b, alloc_bytes);
        //std::cout << "single2 cudaMalloc cost: " << timer0.interval() << " us" << std::endl;
        if (ret != cudaSuccess) {
            std::cout << "cuda alloc dfp32_b failed, msg: " << cudaGetErrorString(ret) << std::endl;
            return -1;
        }
    }
    std::cout << "cuda malloc/memcpy cost: " << timer.interval() << " us" << std::endl;
    lanuchFp16ToFp32Gpu((uint16_t*)dfp16_a, (float*)dfp32_b, VAL_NUM);
    std::cout << "fp16_2_fp32 gpu cost: " << timer.interval() << " us" << std::endl;

    auto ret = cudaMemcpy(fp32_b_0, dfp32_b, alloc_bytes, cudaMemcpyDeviceToHost);
    if (ret != cudaSuccess) {
