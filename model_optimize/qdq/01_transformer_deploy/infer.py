#!/usr/bin/env python3
"""⑤ 用 TensorRT engine 推理。

用法:
    python3 infer.py --engine outputs/model_int8.plan --batch 32
    python3 infer.py --engine outputs/model_fp32.plan --batch 1 --source random
    python3 infer.py --engine outputs/model_int8.plan --batch 32 --compare   # 与 torch 模型对拍
"""
...
def main() -> int:
    ...
    ap.add_argument("--compare", action="store_true", help="与 PyTorch 模型对拍")
    args = ap.parse_args()
    ...
    print(f"  前 8 个 logit : {logit[:8].tolist()}")
    print(f"  前 8 个 prob  : {[round(x, 4) for x in prob[:8].tolist()]}")
    ...
    if args.compare:
        ...
        with torch.no_grad():
            ref = model(**{k: inputs[k] for k in INPUT_NAMES})
        diff = (ref - logit).abs()
