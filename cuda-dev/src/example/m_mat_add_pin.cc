...
int main() {
    ...
    {
        auto ret = cudaMallocHost((void**)&mat_a, alloc_bytes);
        if (ret != cudaSuccess) {
            std::cout << "cuda allocHost mat_a failed, msg: " << cudaGetErrorString(ret) << std::endl;
            return -1;
        }
    }
    
    {
        auto ret = cudaMallocHost((void**)&mat_b, alloc_bytes);
        if (ret != cudaSuccess) {
            std::cout << "cuda allocHost mat_b failed, msg: " << cudaGetErrorString(ret) << std::endl;
            return -1;
        }
    }
    
    {
        auto ret = cudaMallocHost((void**)&mat_c_0, alloc_bytes);
        if (ret != cudaSuccess) {
            std::cout << "cuda allocHost mat_c_0 failed, msg: " << cudaGetErrorString(ret) << std::endl;
            return -1;
        }
    }
