...
被脚本 import（推荐，profile 完直接修，不用记第二件事）：
  ...
  normalize_chrome_trace('trace_chrome.json', ns_to_us=True, drop_outliers=False)
...
def normalize_chrome_trace(
    ...
    *,
    ...
    drop_outliers: True 剔除离群 ts / 异常 dur 的事件（默认）
    ns_to_us:      True 强制按纳秒→微秒换算；False 强制不换算；None 按 span 量级自动判定
    inplace:       True 覆盖原文件；False 写到 out_path（默认 <同名>_fixed.json）
    """
    with open(path) as f:
        data = json.load(f)
